...
# num_jitters= 50 `num_jitters` specifies how many slight variations of an image are created to improve a model’s accuracy in recognizing faces under different conditions.
...
try:
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    # Save screenshot
                    screenshot_filename = f"screenshot/Dinesh datta_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                ...
                for face_location in face_locations:
                    ...
                    cv.putText(frame, 'Dinesh', (left, top - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5,
                               (255, 0, 0), 2, cv.LINE_AA)

        if not recognized:
            # If the face is not recognized, label it as 'Not Barack Obama'
            cv.putText(frame, 'Not Dinesh', (30, 55), cv.FONT_HERSHEY_SIMPLEX, 1,
                       (255, 0, 0), 2, cv.LINE_AA)
    ...
    if not df.empty:
        df.to_excel('excel_sc.xlsx', index=False)
        print("Attendance and screenshots saved to 'excel_sc.xlsx'.")
